...
public class GameModeSurvival : MonoBehaviour
{
    ...
    /// <summary>Запускає рівно одну хвилю (викликається з тригера або кнопки).</summary>
    ...
    {
        if (waveText) waveText.text = "Хвиля " + Mathf.Max(1, CurrentWave);
    }

    private void UpdateAliveUI()
    {
        if (enemiesLeftText) enemiesLeftText.text = "Ворогів: " + AliveEnemies;
    }
